...
//Effective number of Inputs : 3
//Declare INPUT pins
const unsigned int pins_input[] = {2, 3, 6, 4, 5, 6, 2, 3, 4, 5, 6, 2, 3, 4, 5};

//Effective number of Outputs : 8
//Declare OUTPUT pins
const unsigned int pins_output[] = {8, 8, 9, 10, 11, 12, 13, 7, 7, 8, 9, 10, 11, 12, 13, 7};
...
//const unsigned int TU = 1; // TU =5 i.e. 5Hz : Number of callback function of Timer1 per second

void initializeTimer1(){
	if(DEBUG)Serial.begin(9600);
	//Timer1.initialize(1000000);     // initialize timer1, and set a 1 second period
	unsigned int TU = 1000/Board_TIME_UNIT; //FT = frequency_timer
	Timer1.initialize(1000000/TU);       // initialize timer1, and set a 1 second period
	...
	switch (number)
	{
		...
		case 3: //RISING EDGE(^) 0x3000
		{
			...
			*stack_position = !old_RE_value & stack_v1;
			...
			Serial.print("old_RE_index = "); Serial.println(old_RE_index);
			Serial.print("stack_v1 = "); Serial.println(stack_v1);
			delay(600);
...
void EEPROM_Read()
{
	...
	/*
	if(DEBUG){
		//Those informations can not be printed because they are in setup. Move them in the loop for example to make them work.
...
unsigned int readNextIntFromEEPROM()
{
	unsigned int val; //variable de type int, vide, qui va contenir le r�sultat de la lecture
