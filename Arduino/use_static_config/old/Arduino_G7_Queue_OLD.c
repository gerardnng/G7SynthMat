//OLD: Because it contains all comments
/*
//Declare Procedures //Declare Binary Functions
void initializeTimer1()
void Step_timer_update_callback()
void Output_Eval_Update(void)
...
boolean stack_recept_eval[5]; //Old: 4. But it seams that small RAM could necessitates a size greater than 4 
boolean* stack_position = stack_recept_eval; //bool same as boolean

unsigned char* P_Buffer;

unsigned int X; //Actual situation
unsigned int OUT_Port; //Output 'register'
...
const unsigned int frequency_timer = 1; // frequency_timer =5 i.e. 5Hz : Number of callback function of Timer1 per second

void initializeTimer1(){
  Serial.begin(9600);
  //Timer1.initialize(1000000);     // initialize timer1, and set a 1 second period
  Timer1.initialize(1000000/frequency_timer);       // initialize timer1, and set a 1 second period
...
/*
Serial.print("Input state num "); 
Serial.print(i);
Serial.print("= "); 
Serial.println(getBitWithIndex(IN_Port, i));
*/
...
void loop(){
  for(i=0; i<N_Inputs;i++){
    ...
    //Serial.print("Input number "); Serial.print(pins_input[i]);Serial.print(" is active"); Serial.println(getBitWithIndex(IN_Port, i));
  }
  ...
  //Le franchissement est simultané après calcul des transitions franchissables
  ...
  //Step_timer_update_callback(); Because it is called by Timer1
  Output_Eval_Update();
  //Delay to measure step active duration
  //delay(delay_loop); //To uncomment to set a frequence to the program
...
void Step_timer_update_callback()
{
  //Reinitialize to 0 all the active duration of disabled steps 
  //Serial.println("---In Step_timer_update_callback");
  for (i = 0; i < N_Steps; i++)
  {
...
void Output_Eval_Update(void)
{
  //Serial.println("\n---> Output_Eval_Update ... \n");
  unsigned int XMA;
  //temp_IN_OUT_Reg;//starts by 0x0001 to move to 0x8000 (selecting bits)
  OUT_Port = 0x0000;
  for (i = 0; i < N_Actions; i++)
  {
    ...
    if (XMA != 0)
    {
      /*Serial.print("Activate ACTION i = "); Serial.println(i);
      Serial.print("Before: OUT_Port = "); Serial.println(OUT_Port,BIN);
      Serial.print("Before: temp_IN_OUT_Reg = "); Serial.println(temp_IN_OUT_Reg, BIN);
      */
      temp_IN_OUT_Reg = 1<<i;
      OUT_Port = asmOR(OUT_Port,temp_IN_OUT_Reg);
      //Serial.print("After: OUT_Port = "); Serial.println(OUT_Port,BIN);
    }

    /*
    unsigned int a_bit = getBitWithIndex(OUT_Port,i);
    Serial.print("OUT_Port["); 
    Serial.print(i);
    Serial.print("] = ");
    Serial.println(a_bit);
    */
  }
  ...
  for(i=0; i<N_Actions;i++){
    old_state = digitalRead(pins_output[i]);
    new_state = getBitWithIndex(OUT_Port,i);
    if(old_state != new_state){
      digitalWrite(pins_output[i], new_state); 
    }
  }*/
  for(i=0; i<N_Actions;i++){
    digitalWrite(pins_output[i], getBitWithIndex(OUT_Port,i));
  }
}
...
void Operator_Parse(unsigned int a)
{
  ...
  //Serial.print("Operator_Parse is called  with a = "); Serial.println(a);
  ...
  switch (number)
  {
    ...
    {
      //Get old value
	  old_RE_index = RS[i][j+1]; //Look ahead in RS to get the index of the current Rising Edge
...
void Input_Parse(unsigned int a)
{
  ...
  //Serial.print("Input parse is called  with b = "); Serial.println(b);
...
void Step_time_Parse(unsigned int a)
{
  ...
  else //Step n° number is active
  {
    ...
    unsigned int current_duration = Step_active_timer[number]/frequency_timer;
...
void EEPROM_Write()
{
  ...
  unsigned int M_size = sizeof(MatricesDATA); //To modify dynamically inside MPLAB
...
void EEPROM_Read()
{
  ...
  if(DEBUG){
    //Serial.print("N_Trans = "); 
    Serial.println(N_Trans, HEX);
    Serial.print("N_Steps = "); Serial.println(N_Steps, HEX);
    Serial.print("N_Actions = "); Serial.println(N_Actions, HEX);
    Serial.print("N_Inputs = "); Serial.println(N_Inputs, HEX); 
...
/*
unsigned int asmRLNC(unsigned int reg_name, unsigned int n){
  //Move from right to left or simply multiply by 2
  return reg_name<<((asmAND(n,0x000F));  
}
unsigned int asmRRNC(unsigned int reg_name, unsigned int n){
