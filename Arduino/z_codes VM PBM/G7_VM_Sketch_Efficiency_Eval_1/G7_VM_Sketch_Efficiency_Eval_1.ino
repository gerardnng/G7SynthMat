/*
Time :	13/08/2016 12:50:57
Generated from the grafcet File: 
	D:\DD2\THESE\aContinuation_Fev2016\ArduinoCompiler_VM_Code\Efficiency_v2\g7s\Efficiency_Eval_1.xml
*/
#include "TimerOne.h"
#include "EEPROM.h"
...
//RS matrix 
	0xE114, 0x0001, 
	0x8000, 0x8001, 0x1000, 0xE014, 0x0000, 0x0005
};
//From grafcet source File: D:\DD2\THESE\aContinuation_Fev2016\ArduinoCompiler_VM_Code\Efficiency_v2\g7s\Efficiency_Eval_1.xml
...
void setup(){
  Serial.begin(9600);
  Serial.println("gerard nzebop: Step 1");
...
void EEPROM_Write()
{
  Serial.println("gerard nzebop: Step 2");
  
  MatricesDATA_eeprom_adr = 0;
  unsigned int M_size = sizeof(MatricesDATA)/2; //To modify dynamically inside MPLAB

  Serial.print("M_size : ");Serial.println(M_size);
  
  for(i=0; i<M_size ; i++){
    writeNextIntInEEPROM(MatricesDATA[i]);
    Serial.print("gerard nzebop: Step 2_loop : ");Serial.println(i);
  }
  
  Serial.println("gerard nzebop: Step 3");
}

void EEPROM_Read()
{
  Serial.println("gerard nzebop: Step 4");
  MatricesDATA_eeprom_adr = 0;
